...
import org.reactfx.util.AccumulationFacility.Queuing;
...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly) {
		...
		// System.out.println(dict.isWord("ethos"));
		for (int index = 0; index <= s.length(); index++) {
			for (int charCode = (int) 'a'; charCode <= (int) 'z'; charCode++) {
				// use StringBuffer for an easy interface to permuting the
				// letters in the String
...
//		System.out.println("done");
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		queue.add(word);
		visited.add(word);
		while(!queue.isEmpty() && numSuggestions>0 && thres<THRESHOLD) {
			String curr = queue.remove(0);
			thres++;
//			System.out.println("Curr is " + curr);
			List<String> neighbors = distanceOne(curr, true);
			for (String neigh: neighbors) {
				if(!visited.contains(neigh) && numSuggestions>0) {
					visited.add(neigh);
					queue.add(neigh);
					if (dict.isWord(neigh)) {
//						System.out.println("adding " + neigh);
						retList.add(neigh);
						numSuggestions--;
					}
				}
...
//		  NearbyWords any Dictionary implementation you prefer 
		  ...
		  word = "tailo"; 
		  List<String> suggest = w.suggestions("kangaro", 10);
